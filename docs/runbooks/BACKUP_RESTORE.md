...
|---|---|---|---|---|---|---|
| 1 | 2026-09-14 | **الإنتاج** — نسخة مستوى التطبيق `GET /system/backup` (162 صفًا، 58,969 بايت) | **PASS** (بعد إصلاح عيبين اكتشفتهما البروفة — فرع `fix/backup-restore-dr-gate`) | ~25 | Super Z (بطلب المالك) | انظر «سجل بروفة مستوى التطبيق» أدناه |
| 2 | | | | | | |
| 3 | | | | | | |

> **قاعدة:** بروفة فاشلة أو شهر بلا بروفة ⇒ يُعاد فتح بند «backup/restore مجربان» في بوابة G9→G10 حتى نجاح بروفة جديدة.

### سجل بروفة مستوى التطبيق — 2026-09-14 (التفصيل)

**النطاق:** هذه بروفة **مستوى التطبيق** (ميزة النسخ/الاستعادة من محاكاة Selim W2) — مكملة لبروفة pg_dump في §4 وليست بديلًا عنها؛ بروفة pg_dump على قاعدة الإنتاج تتطلب وصول DB مباشرًا (Railway) وهو بيد المالك.

**الخطوات المنفذة والنتائج:**

1. **النسخة:** `GET /system/backup` من الإنتاج بحساب SUPER_ADMIN — 200، 58,969 بايت، 162 صفًا في 28 جدولًا، `formatVersion: 1`.
2. **قاعدة البروفة:** PostgreSQL 16.15 محمول (منفذ 5433) — `migrate deploy`: **43/43 هجرة** ناجحة، 0 معلقة، 76 جدول `public`.
3. **الاستعادة (محاولة 1):** فشلت بـ P2003 — **اكتشاف عيب 1**: `FiscalPeriod` كان يُدرج قبل `User` رغم `fiscal_periods.createdById → users` (+ عيبان كامنان: `Voucher` قبل `JournalEntry` و`Quotation` قبل `SalesOrder` + مرجعان ذاتيان بلا فرز).
4. **الاستعادة (محاولة 2 بعد إصلاح الترتيب):** فشلت بـ P2003 من سجل التدقيق — **اكتشاف عيب 2**: `SYSTEM_RESTORED` يُنسب لمعرف المنفِّذ الذي يحذفه الـ TRUNCATE نفسه ⇒ الاستعادة على نظام جديد (سيناريو DR حرفيًا) كانت **مستحيلة**.
5. **الاستعادة (محاولة 3 بعد الإصلاحين):** ✅ نجحت — 28 جدولًا / 162 صفًا مطابقة تمامًا لعدادات `meta` في ملف النسخة.
6. **تسجيل دخول فعلي على النسخة المستعادة** ببيانات مدير الإنتاج: ✅ 200 (`المدير العام` / SUPER_ADMIN) — أقوى دليل أن البيانات المستعادة صالحة تشغيليًا.
7. **الفحوصات الإلزامية:** جداول public = 76 · `_prisma_migrations` = 43 (يطابق مجلدات الهجرات) · 0 هجرات معلقة · عدادات كل الجداول مطابقة للنسخة · لوحة المؤشرات (`/dashboard/stats`) **متطابقة** بين الإنتاج والنسخة المستعادة (الفروق الوحيدة: طوابع زمنية).
8. **Reconciliation محاسبي:** سطور القيود بلا حسابات ناقصة أو مبالغ غير موجبة · ميزان المراجعة المحسوب من القيود = الأرصدة المخزنة (كلاهما صفر — الإنتاج بلا قيود) · سلسلة `balanceAfter` في دفتر المخزون متصلة 100% · لا أرصدة سالبة.
9. **الاختبارات بعد الإصلاح:** 9/9 في وحدة النسخ الاحتياطي (3 اختبارات جديدة: حارس ترتيب FK يقرأ schema + سيناريو DR + بلا `exportedById`) · المجموعة الكاملة **74/74 suites، 1064/1064 tests** · typecheck/lint/format خضراء.

**اكتشاف محاسبي (يُبلَّغ للمالك):** الإنتاج يحمل مخزون خامات بقيمة **7,005 EGP** (150م قماش × 45.50 + 12 بكرة × 15.00) ومنتج تام (50+30 وحدة) **بلا أي قيد افتتاحي** (قيود اليومية = 0 وكل أرصدة الحسابات = 0.00) — بذرة الإنتاج (نسخة قديمة) لا تنشئ قيد الرصيد الافتتاحي بينما البذرة الحالية تفعله محليًا. **المطلوب من المالك:** ترحيل قيد افتتاحي (Dr INVENTORY / Cr OWNERS_EQUITY = 7,005) في الإنتاج أو إعادة بذر نظيفة.

**الخلاصة للمصادقة:** مستوى التطبيق = **مجرب ويعمل بعد الإصلاحين**؛ مستوى pg_dump (§3/§4) لا يزال ينتظر وصول DB من المالك.
