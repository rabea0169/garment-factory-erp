...
## إغلاق الأمن والعمليات — صفر ثغرات + كسر P0 في القيود اليدوية + أدوات المراقبة والنسخ الخارجي (2026-09-16)

**القرار (المالك):** «حل كل المشاكل» — إغلاق ثغرة mysql2/Prisma المؤجلة
وبقية بنود npm audit، وإصلاح كل ما يُعطل المسار المحاسبي.

**1) إغلاق npm audit كاملًا (صفر ثغرات):**
- `mysql2` 3.15.3 → **3.24.4** عبر `overrides` (كانت مؤجلة لأن npm يقترح
  تنزيل Prisma إلى 6 — كاسرًا؛ الـ override يبقى داخل 3.x فلا يمس مسار
  PostgreSQL إطلاقًا).
- `multer` 2.2.0 → **2.4.0** عبر `overrides` (4 ثغرات High — مسار فعلي:
  `POST /system/restore` يستخدم FileInterceptor؛ التخفيف كان قائمًا
  SUPER_ADMIN + حد حجم، والآن الثغرات نفسها مغلقة).
- `exceljs` → `uuid` **11.1.1** (override scoped — exceljs وحده يعتمد على
  uuid في الشجرة كلها).
- `js-yaml` → 4.3.2 عبر `npm audit fix` غير الكاسر.
- **النتيجة: `npm audit` = 0 vulnerabilities** (كانت 9-10: 1-3 moderate
  و7-8 high). قرار MASTER_BACKLOG القديم بالتأجيل يُغلق — لم يعد له موضوع.

**2) اكتشاف وإصلاح كسر P0 في القيود اليدوية على حسابات النظام (ACC):**
- **العيب:** ترقية `class-validator` إلى 0.15.1 (مع validator.js المحدَّث)
  جعلت `@IsUUID()`/`@IsUUID(undefined)` ترفض معرفات شجرة الحسابات النظامية
  الثابتة (CHART_OF_ACCOUNTS — نسق version-0 مثل
  `10000000-0000-0000-0000-000000000031`). هذه المعرفات صالحة تمامًا لعمود
  uuid في PostgreSQL، لكن validator.js الجديد يشترط version nibble ‏1-8.
- **الأثر:** أي قيد يدوي على حساب نظامي (مخزون/نقدية/عملاء/موردون/حقوق
  ملكية — 19 حسابًا) عبر `POST /accounting/journal-entries` = **400**
  «must be a UUID». أُثبت كسرًا فعليًا على الإنتاج أثناء محاولة ترحيل
  القيد الافتتاحي. المواضع الأربعة المتأثرة كلها أُصلحت: بنود القيد
  (JournalLineDto)، حساب الأب (CreateAccountDto.parentId)، قوالب القيود،
  وخطط الموازنات.
- **الإصلاح:** `@IsUUID('loose')` — نسق 8-4-4-4-12 hex كاملًا (نفس شرط
  عمود uuid أصلًا)؛ رفض القمامة وغير-hex يبقى راسخًا. اختبارا انحدار
  جديدان يثبتان القبول والرفض.
- **ملاحظة منهجية:** البذرة والخدمات الداخلية لم تتأثر (لا تمر عبر DTO) —
  لذلك بقيت 1064 اختبارًا خضراء بينما المسار الحرج على الإنتاج كان مكسورًا؛
  فجوة «اختبارات DTO بواجهة HTTP» سُدّت الآن.

**3) أدوات العمليات (مُختبرة ضد الإنتاج مباشرة):**
- `backend/scripts/monitor-health.mjs`: فحص 4 طبقات متدرجة (health/ready/
  login/backup-summary) برمز خروج مناسب لـ cron وخيار webhook تنبيه —
  اختُبر على الإنتاج: **4/4 PASS** (76 جدولًا/178 صفًا آنذاك).
- `backend/scripts/backup-download.mjs`: النسخة الخارجية المؤتمتة
  (تسجيل دخول → `GET /system/backup` → ملف مؤرخ + sha256 + تدوير retention
  30 يومًا + تحقق سلامة قبل الحفظ) — اختُبر على الإنتاج: نسخة سليمة
  76 جدولًا/180 صفًا/93.3KB محفوظة بـ sha256. يغلق فجوة «الطبقة الخارجية
  إلزامية» من runbook النسخ، وتبقى جدولة cron خارج الخادم على المالك
  (الأداة جاهزة والباقي تشغيل مجدول).
- **بوابات الفرع:** typecheck/lint/format ✅ + **1066/1066 unit** (+2
  انحدار جدد) + 76/76 e2e + **45/45 integration على PostgreSQL 16 حقيقي**
  (شغّل التحقق الكامل بعد الـ overrides) + `npm audit` صفر.

**4) القيد الافتتاحي (الفجوة المحاسبية المكتشفة 2026-09-14) — نُفِّذ على
الإنتاج ✅:** بعد نشر إصلاح DTO على Railway (إعادة تشغيل مؤكدة 2026-09-15
21:53 UTC) نُفِّذ القيد عبر API بمفتاح idempotency
`opening-entry-2026-09-16-001`:
- **النتيجة المسجلة:** قيد `JE-20260915-D9D4D50C` — **Dr 1300 المخزون /
  Cr 3000 حقوق الملكية = 7,005.00 EGP** (بند واحد، isAuto=false، مرجع
  OPENING-ENTRY-2026-09، الفترة المالية 2026 المفتوحة).
- **التحقق:** totalDebit = totalCredit = 7005 ✓ — إعادة الطلب بنفس المفتاح
  رُفضت 409 (idempotency راسخ — لا تكرار ممكن) ✓ — نسخة احتياطية لاحقة
  تؤكد: قيد واحد وبند واحد بالضبط في القاعدة (76 جدولًا/195 صفًا) ✓.
- **الأثر المحاسبي:** الدفاتر لم تعد أصفارًا — مخزون الخامات 7,005 EGP
  مُقابل حقوق ملكية، متطابق مع حركات ledger الفعلية
  (SLE-SEED-OPENING-001/002). المنتج التام (50+30 وحدة بتكلفة 0) خارج
  القيد بنفس منطق البذرة الحالية (لا قيمة دفترية له).
- بذلك تُغلق «الفجوة المحاسبية» نهائيًا كقرار مالك منفَّذ بالمسار غير
  الكاسر (قيد افتتاحي بدل إعادة البذر التي كانت ستمحو بيانات الإنتاج).
...
| آخر commit على main | `87109a3` — تثبيت objective_c لإصلاح كسر CI على main (#97) بتاريخ 2026-09-14؛ يليه `239b228` (مزامنة الوثائق #96) و`dad5080` (UI-COMPLETE #95) |
...
| الإنتاج (Railway) | `garment-factory-erp-production.up.railway.app` — health/ready 200 وlogin 200 متحقق منهما مباشرة 2026-09-16 (فحص المراقبة 4/4)؛ 76 جدولًا/180 صفًا (نسخة خارجية محفوظة بـ sha256) |
...
| سبب عدم الإغلاق النهائي | UAT ميداني 16/16 على جهاز Android فعلي، بروفة pg_dump (تحتاج وصول DB من المالك)، توقيع APK إنتاجي (keystore)، وقرار إطلاق المالك (G10/G11) — **القيد الافتتاحي نُفّذ على الإنتاج 2026-09-16 (JE-20260915-D9D4D50C)** |
| Security blockers | لا أسرار في المستودع (SEED_ADMIN_PASSWORD متغير بيئة على Railway فقط)؛ **`npm audit` = 0 vulnerabilities** (2026-09-16: mysql2/multer/uuid عبر overrides + js-yaml عبر audit fix — راجع القسم أعلاه) |
| Open decisions | نطاق/توقيت UAT الفعلي؛ آلية توقيع APK (keystore)؛ تشغيل مجدول (cron) لسكربتي المراقبة والنسخ الخارجي لدى المالك |
| Last handoff | سلسلة موجات Selim موثقة في `docs/SELIM_REPLICATION.md` (الأقسام 1-6)؛ آخر PR توثيقي #92 |
| Next exact action | تنفيذ UAT الميداني 16/16 على جهاز Android فعلي بقاعدة الإنتاج وفق docs/UAT_SCENARIOS.md (بعد نشر #98 الذي أغلق الأمن/القيود — متحقق منه حي 2026-09-16) |
