...
**4) القيد الافتتاحي (الفجوة المحاسبية المكتشفة 2026-09-14):** بياناته
محسوبة ومطابقة لحركات ledger الفعلية — **Dr 1300 المخزون / Cr 3000 حقوق
الملكية = 7,005.00 EGP** (RM-001: 150×45.5=6825 + RM-002: 12×15=180 —
مطابقة لـ SLE-SEED-OPENING-001/002؛ المنتج التام 50+30 بتكلفة 0 لا يدخل
القيد — نفس منطق البذرة الحالية). التنفيذ عبر API محجوز **بعد نشر إصلاح
DTO** هذا على Railway (المسار كان مكسورًا قبله) — فور النشر يُنفَّذ بمفتاح
idempotency `opening-entry-2026-09-16-001` ثم يوثَّق هنا بالنتيجة.
...
| Next exact action | دمج PR إغلاق الأمن/العمليات (صفر ثغرات + إصلاح DTO القيود اليدوية + سكربتا المراقبة/النسخ) ← بعد نشر Railway: تنفيذ القيد الافتتاحي عبر API بمفتاح `opening-entry-2026-09-16-001` ثم UAT الميداني وفق docs/UAT_SCENARIOS.md |
