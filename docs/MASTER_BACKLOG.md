...
## GF-IMP — خطة التحسينات الشاملة المعتمدة (بنود الموجات)

> اعتُمدت وثيقة «خطة التحسينات الشاملة» (112 بندًا) بالكامل من المالك 2026-09-06. الحوكمة: معرف GF-IMP لكل بند، PR واحد لكل حزمة، تحديث وثائق الحالة عند كل دمج، وقائمة فحص المراجعة `docs/REVIEW_CHECKLIST.md` إلزامية لكل PR.
...
| GF-IMP-UI | توحيد واجهة المستخدم — مراجعة وتطوير شاملة: توحيد كل الشاشات على الهيكل التكيفي `SelimShellScaffold` (تنقل سفلي جوال + NavigationRail ديسكتوب + لوحة أوامر Ctrl+K + جرس تنبيهات + زر رجوع ذكي GfBackButton)، شاشة دخول مطورة (إظهار كلمة المرور + سكيلتون)، توسيع الهيكل بدعم `bottom` (TabBar) و`bottomBar` (شريط الإجماليات اللزج) | ✅ مكتملة على main عبر PR #94 (`796b9e5` — 12 شاشة + دخول + سكيلتون) ثم PR #95 (`dad5080` — آخر 7 شاشات: كشوف الرواتب، نشاط/تقرير العامل، تسوية جرد، منتج، مرتجع، عرض سعر) — **41/42 شاشة على الهيكل الموحد** (الدخول وحدها خارجًا عن قصد)، CI أخضر 5/5 على الفرع و4/4 على main (2026-09-14) |
...
|---|---|---|---|---|---|
| GF-REMAINING-010 | إصلاح انحراف schema الموجة 4 + إعادة CI أخضر | `backend/prisma/schema.prisma` (إضافة `@map` لـ `jwtVersion` وكل حقول `RefreshToken` — بلا migration جديد لأن بنية القاعدة صحيحة)، `backend/test/auth-guard.e2e-spec.ts`، `backend/test/production-workflow-api.e2e-spec.ts` | migration `20260902000000_wave4_sec_f04_refresh_tokens`؛ تصحيح CI الأحمر على آخر 8 تشغيلات منذ 2026-08-28 | seed ناجح على قاعدة نظيفة (خطوة Seed في CI) + login يعمل (200) + E2E 64/64 + CI أخضر على main بعد الدمج | ✅ مدمجة في `main@d6ad465` عبر PR #77؛ CI أخضر (تشغيل 33963217026) والتحقق مكتمل: seed + login 200 + دورة SEC-F04 كاملة على الإنتاج نفسه |
| GF-REMAINING-001 | حماية ProductsController الحساسة واختبار RBAC | `backend/src/modules/products/products.controller.ts`، الاختبارات، `docs/API_CONTRACT.md` | خط الأساس `ab8f87d` | كل مسار تعديل/حذف/BOM يحمل دورًا مناسبًا؛ `401` بلا token و`403` للدور الخطأ ونجاح للدور الصحيح، مع تحقق DTO | ✅ مدمجة عبر PR #48 (`dae390dd`) — أدوار RBAC على مسارات تعديل/حذف/BOM للمنتجات موجودة على main (تحقق 2026-09-15) |
